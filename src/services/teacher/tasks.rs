...
use sea_orm::{
    query::*, ActiveModelTrait, ColumnTrait, DatabaseConnection, EntityTrait, IntoActiveModel,
    QueryFilter, RelationTrait, Set, TransactionTrait,
};
use validator::Validate;

#[get("/tasks")]
pub async fn get_teacher_tasks(
    req: HttpRequest,
    pool: web::Data<DatabaseConnection>,
    query: web::Query<subjects::ModelQuery>,
) -> impl Responder {
...
#[post("/tasks")]
pub async fn post_teacher_tasks(
    req: HttpRequest,
    pool: web::Data<DatabaseConnection>,
    data: web::Json<assignments::ModelIdLess>,
) -> impl Responder {
    let ext = req.extensions();
    let account = match ext.get::<Account>() {
        Some(acc) => acc,
...
#[put("/tasks")]
pub async fn put_teacher_tasks(
    req: HttpRequest,
    pool: web::Data<DatabaseConnection>,
    data: web::Json<assignments::Model>,
) -> impl Responder {
    let ext = req.extensions();
    let account = match ext.get::<Account>() {
        Some(acc) => acc,
...
#[delete("/tasks")]
pub async fn delete_teacher_tasks(
    req: HttpRequest,
    pool: web::Data<DatabaseConnection>,
    data: web::Json<assignments::ModelId>,
) -> impl Responder {
    let ext = req.extensions();
    let account = match ext.get::<Account>() {
        Some(acc) => acc,
